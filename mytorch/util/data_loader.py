...
class DataLoader:
    ...
    def load(self, train_batch_size: int = 500, test_batch_size: int = 100):
        ...
        for i in range(TRAIN):
            label = int(i / 1000)
            index = int((i % 1000) + 1)
            addr = f"{self.train_addr}/{label} ({index}).jpg"
            img = Image.open(addr).convert('L')  # Ensure image is in grayscale
            img = img.resize((28, 28))  # Ensure the image size is 28x28
            train_data.append((np.array(img), label))
        ...
        for i in range(TEST):
            label = int(i / 100)
            index = int((i % 100) + 1)
            addr = f"{self.test_addr}/{label} ({index}).jpg"
            img = Image.open(addr).convert('L')  # Ensure image is in grayscale
            img = img.resize((28, 28))  # Ensure the image size is 28x28
            test_data.append((np.array(img), label))
